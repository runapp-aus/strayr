...
#'
#' @param x a (character) vector containing ASCED levels of education.
#' Note that clean_asced_qual always returns a character vector. If no match is found, then NA is returned.
#'
...
#'
#' @seealso  \code{\link{clean_asced_foe}} for ASCED fields of education, \code{\link{clean_anzsco}} for ANZSCO, \code{\link{clean_anzsic}} for ANZSIC,  \code{\link{clean_state}} for Australian states and territories.
#'
#' @examples
#'
#' clean_asced_qual("Graduate diploma")
#'
#' clean_asced_qual("Grad dip", fuzzy_match = TRUE)
#'
#' @export
#'
#'
clean_asced_qual <- function(x,
                         fuzzy_match = FALSE,
                         max_dist = 0.4,
                         method = "jw",
                         silent = FALSE) {
  clean_titles(
    dictionary = asced_qual_dictionary,
    .vector = x,
